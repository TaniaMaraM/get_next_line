...
/*   Updated: 2025/03/06 16:07:39 by tmarcos          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*ft_update_keep(char *keep)
{
	...
	new_keep = ft_substr(keep, newline_index + 1,
			ft_strlen(keep) - (newline_index + 1));
	if (!new_keep)
	{
		free(keep);
		return (NULL);
...
char	*get_next_line(int fd)
{
	...
	keep[fd] = ft_update_keep(keep[fd]);
	return (line);
}
